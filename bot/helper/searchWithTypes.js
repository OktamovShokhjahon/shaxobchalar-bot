...
const show_stations = async (chatId, page = 1, message_id = null, isMyStations = false, type = undefined) => {
  ...
  newStations.forEach(station => {
    ...
    station.oilTypes.forEach((type) => {
  	...
  	if (newTypes.includes(type.toLowerCase())) {
  		lastResult.push(station)
  	}
  })

  let list = lastResult.map((station) => {
    return [
      {
        text: station.name,
...
const requestType = async (msg) => {
	const chatId = msg.from.id
	const user = await User.findOne({chatId}).lean()

	await User.findByIdAndUpdate(user._id, {...user, action: "request_type_to_search"}, {new: true})

	bot.sendMessage(chatId, "Qaysi tur bo'yicha qidirmoqchi ekanligingizni kiriting.")
}

const searchWithTypes = async (msg) => {
	const chatId = msg.from.id
	const type = msg.text
	...
	show_stations(chatId, 1, null, false, type)
}

module.exports = {searchWithTypes, requestType}