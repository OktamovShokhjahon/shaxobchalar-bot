...
const show_stations = async (chatId, page = 1, message_id = null, isAdmin = false, number = "999999999999") => {
  ...
  stations.forEach((station) => {
      if (isAdmin && station.status) {
      	if (!ids.includes(station._id)) {
        	newStations.push(station)
        	ids.push(station._id)
      	}
      } else {
        if (station.status) {
        	if (number.includes(station.adminNumber)) {
          		if (!ids.includes(station._id)) {
          			newStations.push(station)
          		ids.push(station._id)
          		}
...
const userStations = async (msg) => {
	const chatId = msg.from.id

	const user = await User.findOne({chatId}).lean()

	const {admin, phone} = user

	show_stations(chatId, 1, null, admin, phone)
}
